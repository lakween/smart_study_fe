...
- `ExamDetailScreen`: secure preflight, private collaborative-lobby readiness, invitation response, per-participant contribution counts, blind organizer publishing, cancellation, and start/resume/results actions. Detail and result views stay inside the authenticated shell with the Exams dock item selected; create, question-building, contribution, and active-attempt screens remain focused without the dock.
- `ExamContributionScreen`: an individual-exam-style full-screen picker with search, compact collapsible filters, whole-quiz/per-question actions, selected/available separation, and exact quota progress. A full quiz selection submits directly without a forced review step. Existing submissions appear as a selected **Current private questions** group that can be retained, removed, or replaced. `Write new` remains a secondary collapsible manual editor, prefills any selected questions, validates four distinct options and local duplicates, warns before discarding unsaved edits, and never exposes another participant's content.
