...
Current focused coverage includes compact-phone navigation/exam/performance widgets, shell-owned performance navigation, the grouped exam question-library picker, direct friend contribution selection plus its manual fallback, exam and performance DTO parsing, spaced-repetition transitions, exam lifecycle/scoring/release rules, selectable error presentation, safe error mapping, rate limiting, enum mapping, and database-backed subject deep copy. Continue adding route-level authorization and database integration coverage.
