...
- Build individual exam papers from any questions in the organizer's own quizzes, not only a selected topic. Reuse `ExamQuestionLibraryPicker` for individual papers and friend contributions: support search plus subject/topic/quiz filters, whole-quiz and per-question selection, separate Selected quizzes and Available quiz library sections, and quota-aware selection for friend contributions. Keep these as sections in one view, not tab controls. After an individual exam is published, lock its question paper; allow cancellation through the normal published-exam lifecycle only.
