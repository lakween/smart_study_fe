...
class _CreateQuizScreenState extends ConsumerState<CreateQuizScreen> {
  ...
  void _addQuestion() =>
      setState(() => _questions.add(_QuestionForm(id: const Uuid().v4())));
  ...
  Future<void> _save() async {
    ...
    if (_questions.isEmpty) {
      AppMessage.error(context, 'Add at least one question');
      return;
    }
    if (_topicId == null) {
      AppMessage.error(context, 'Please select a topic');
      return;
    }
    ...
    if (ok) {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
          content: Text(isEditing ? 'Quiz updated!' : 'Quiz created!'),
                    ...
                    children: [
                      ...
                      ..._questions.asMap().entries.map((e) {
                        ...
                        return _QuestionCard(
                          ...
                          onMoveUp:
                              i == 0 ? null : () => _moveQuestion(i, i - 1),
                          onMoveDown: i == _questions.length - 1
                              ? null
                              : () => _moveQuestion(i, i + 1),
                          onCorrectChanged: (ans) =>
                              setState(() => q.correct = ans),
                        );
                      }),
                      if (_questions.isEmpty)
                        Container(
                      ...
                      AppButton(
                          label: isEditing ? 'Save Changes' : 'Save Quiz',
              ...
              children: AnswerOption.values
                  .map((opt) => Padding(
                        ...
                        child: Row(
                          children: [
                            Radio<AnswerOption>(
                                value: opt, activeColor: AppColors.primary),
                            Expanded(
                              child: TextFormField(
                                controller: switch (opt) {
                                  AnswerOption.a => form.optA,
                                  AnswerOption.b => form.optB,
                                  AnswerOption.c => form.optC,
                                  AnswerOption.d => form.optD
                                },
                                decoration: InputDecoration(
