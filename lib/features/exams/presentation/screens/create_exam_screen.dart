...
class _CreateExamScreenState extends ConsumerState<CreateExamScreen> {
  ...
  Future<void> _pickDateTime() async {
    ...
    final time = await showTimePicker(context: context, initialTime: TimeOfDay.fromDateTime(DateTime.now().add(const Duration(hours: 1))));
    if (time == null) return;
  ...
  Future<void> _create() async {
    ...
    if (_type == ExamType.friendExam && _selectedFriends.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Select at least one friend to invite'), backgroundColor: AppColors.error));
      return;
    }
    ...
    final ok = await ref.read(examProvider.notifier).createExam(
      ...
      participantIds: _type == ExamType.friendExam ? _selectedFriends.toList() : const [],
    );
    if (!mounted) return;
          ...
          child: Form(
            ...
            child: Column(
              ...
              children: [
                ...
                Text('Exam Type', style: theme.textTheme.titleSmall?.copyWith(fontWeight: FontWeight.w600)),
                ...
                const SizedBox(height: 20),
                Text('Duration', style: theme.textTheme.titleSmall?.copyWith(fontWeight: FontWeight.w600)),
                const SizedBox(height: 10),
                Wrap(
                  ...
                  child: Container(
                    ...
                    child: Row(
                      children: [
                        ...
                        Text(_startTime != null ? '${_startTime!.day}/${_startTime!.month}/${_startTime!.year} ${_startTime!.hour}:${_startTime!.minute.toString().padLeft(2, '0')}' : 'Select start time',
                          style: TextStyle(color: _startTime != null ? null : AppColors.textMuted)),
                      ],
                    ),
                  ),
                ),
                if (_type == ExamType.friendExam) ...[
                  const SizedBox(height: 20),
                  Text('Invite Friends', style: theme.textTheme.titleSmall?.copyWith(fontWeight: FontWeight.w600)),
                  const SizedBox(height: 4),
                  const Text('Each participant will get randomised questions', style: TextStyle(fontSize: 12, color: AppColors.textMuted)),
                  const SizedBox(height: 10),
                    ...
                    Wrap(
                      ...
                      children: _selectedFriends.map((id) {
                        ...
                        return Chip(
                          ...
                          onDeleted: () => setState(() => _selectedFriends.remove(id)),
                ...
                AppButton(label: 'Create Exam', onPressed: _saving ? null : _create, isLoading: _saving),
              ],
            ),
          ),
        ),
