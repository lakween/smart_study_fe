...
class _AiQuizScreenState extends ConsumerState<AiQuizScreen> {
  ...
  Future<void> _generate() async {
    ...
    try {
      ...
      if (mounted) {
        setState(() { _generating = false; _step = 0; _genError = apiErrorMessage(e, fallback: 'Could not generate quiz from this document'); });
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(_genError!), backgroundColor: AppColors.error));
      }
    } finally {
  ...
  Future<void> _saveQuiz() async {
    ...
    if (ok) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('AI Quiz saved!'), backgroundColor: AppColors.success));
                  ...
                  children: [
                    Container(
                      ...
                      child: Center(child: i < _step
                          ...
                          : Text('${i + 1}', style: TextStyle(fontSize: 11, fontWeight: FontWeight.bold, color: i <= _step ? Colors.white : AppColors.textMuted))),
                    ),
                    const SizedBox(width: 6),
                    Text(['Upload', 'Generate', 'Review'][i], style: TextStyle(fontSize: 11, fontWeight: i == _step ? FontWeight.w700 : FontWeight.normal, color: i == _step ? AppColors.primary : AppColors.textMuted)),
                    if (i < 2) Expanded(child: Container(height: 2, margin: const EdgeInsets.symmetric(horizontal: 4), color: i < _step ? AppColors.primary : AppColors.divider)),
                  ],
                ),
              )),
            ),
                ...
                SingleChildScrollView(
                  ...
                  child: Column(
                    ...
                    children: [
                        ...
                        child: Container(
                          ...
                          child: _file != null
                              ? Column(children: [
                                  ...
                                  Text(_file!.name, style: const TextStyle(fontWeight: FontWeight.bold)),
                                  Text('${(_file!.size / 1024 / 1024).toStringAsFixed(2)} MB', style: const TextStyle(color: AppColors.textMuted, fontSize: 12)),
                                  const SizedBox(height: 8),
                                  TextButton(onPressed: _pickFile, child: const Text('Change file')),
                                ])
                              : const Column(children: [
                                  Icon(Icons.upload_file, size: 48, color: AppColors.textMuted),
                                  SizedBox(height: 8),
                                  Text('Tap to select document', style: TextStyle(fontWeight: FontWeight.w600, color: AppColors.textSecondary)),
                                  SizedBox(height: 4),
                                  Text('PDF, JPG, PNG, JPEG', style: TextStyle(color: AppColors.textMuted, fontSize: 12)),
                                ]),
                        ),
                      ),
                      ...
                      const SizedBox(height: 20),
                      Text('Number of Questions', style: theme.textTheme.titleSmall?.copyWith(fontWeight: FontWeight.w600)),
                ...
                Center(
                  child: Padding(
                    ...
                    child: Column(
                      ...
                      children: [
                        const Icon(Icons.auto_awesome, size: 72, color: AppColors.primary)
                          .animate(onPlay: (c) => c.repeat()).scale(begin: const Offset(0.8, 0.8), end: const Offset(1.2, 1.2), duration: 800.ms, curve: Curves.easeInOut),
                        const SizedBox(height: 24),
                        const Text('Generating your quiz...', style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold)),
                        const SizedBox(height: 8),
                        Text(_generating ? _genMessages[_genStage.clamp(0, _genMessages.length - 1)] : 'Done!',
                        ...
                        children: [
                          Container(
                            ...
                            child: const Row(
                              children: [
                                ...
                                SizedBox(width: 8),
                                Expanded(child: Text('AI Generated — Please review before saving', style: TextStyle(color: AppColors.warning, fontWeight: FontWeight.w600, fontSize: 13))),
                              ],
                            ),
                          ),
                          const SizedBox(height: 16),
                          ..._questions.asMap().entries.map((e) {
                            ...
                            return Container(
                              ...
                              decoration: BoxDecoration(color: Theme.of(context).brightness == Brightness.dark ? AppColors.darkCardBg : AppColors.cardBg, borderRadius: BorderRadius.circular(12), boxShadow: AppColors.cardShadow),
                              child: Column(
                                crossAxisAlignment: CrossAxisAlignment.start,
                                children: [
                                  Row(
                                  ...
                                  TextFormField(controller: q.text, maxLines: 2, decoration: const InputDecoration(labelText: 'Question', filled: true, border: InputBorder.none), style: const TextStyle(fontSize: 13)),
                                  const SizedBox(height: 8),
                                  ...AnswerOption.values.map((opt) => Row(
                                    children: [
                                      Radio<AnswerOption>(value: opt, groupValue: q.correct, onChanged: (v) => setState(() => q.correct = v!), activeColor: AppColors.primary, materialTapTargetSize: MaterialTapTargetSize.shrinkWrap),
                                      Expanded(child: TextFormField(
                                        controller: switch(opt) { AnswerOption.a => q.optA, AnswerOption.b => q.optB, AnswerOption.c => q.optC, AnswerOption.d => q.optD },
                                        decoration: InputDecoration(labelText: opt.label, filled: true, border: InputBorder.none, contentPadding: const EdgeInsets.symmetric(horizontal: 8, vertical: 6)),
                          ...
                          TextButton.icon(icon: const Icon(Icons.add), label: const Text('Add More Questions'), onPressed: () => setState(() => _questions.add(_EditableQuestion(id: const Uuid().v4(), qText: '', a: '', b: '', c: '', d: '', correct: AnswerOption.a)))),
                          const SizedBox(height: 16),
                          AppButton(
                            label: 'Save Quiz',
                            isLoading: _saving,
                          ...
                          const SizedBox(height: 12),
                          AppButton(label: 'Discard', variant: AppButtonVariant.outlined, onPressed: () => setState(() { _step = 0; _questions.clear(); })),
                        ],
                      ),
              ],
            ),
